...
export const deleteHealthStatusByPK = (args, result) => {
    db.query("DELETE FROM healthstatus WHERE PID = ?, fillOutDate = ?", [args.pid, args.fillOutDate], (err, results) => {
        if(err) {
            console.log(err);
            result(err, null);
        } else {
...
export const updateHealthStatusByPK = (data, pid, fillOutDate, result) => {
    db.query("UPDATE healthstatus SET ? WHERE PID = ?, fillOutDate = ?", [data, pid, fillOutDate], (err, results) => {
        if(err) {
            console.log(err);
            result(err, null);
        } else {
