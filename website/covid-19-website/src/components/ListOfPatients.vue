    ...
    async getPatients() {
      ...
      try {
        if(this.userRole == "doctor"){
          ...
          const response = await axios.get(`http://localhost:5001/usersByCovid`);
          this.patientList = response.data;
          const travelResponse = await axios.post(`http://localhost:5001/users`, {Role: 'Patient', Travelled: 1});
